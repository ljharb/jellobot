...
const installDocker = async function ({ ssh, log }) {
	try {
	...
	} catch (e3) { // eslint-disable-line no-unused-vars
		// Do nothing
	}
};
...
const run = async function () {
	...
	try {
		...
		} catch (e4) { // eslint-disable-line no-unused-vars
			// do nothing
		}
		...
		await installGeneralDeps({ log, ssh });
		await installPython({ log, ssh });
		await installNode({ log, ssh });
		await installPm2({ log, ssh });
		await installDocker({ log, ssh });
	} finally {
		await ssh.end();
	}
};
