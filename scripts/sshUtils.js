...
class SshConnection {
	...
	async exec(_command, _opts) {
		...
		return new Promise((resolve, reject) => {
			...
			command = this.validAndFixCmd(command, opts);
			...
			this.conn.exec(command, execOpts, (err, stream) => {
				...
				if (!opts.silent) {
					...
					if (opts.name) {
						tempLog = `Running command on ${opts.name}: ${command}`;
					}
