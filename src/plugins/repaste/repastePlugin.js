...
const matchUrl = (text) => {
	...
	const match = text.match(/(?:http|https):\/\/[\w-]+(?:\.[\w-]+)+(?:[\w.,@?^=%&amp;:/~+#-]*[\w@?^=%&amp;/~+#-])?/);
	if (match) {
		return match[0];
	}
	return match;
...
const findLinkInLogs = (msg, user) => {
	for (let i = 0; i < msg.logs.length; i += 1) {
		...
		if (!user || log.from === user) {
			...
			if (match) {
				return { url: match, user: log.from };
			}
		}
	}
	return [];
...
const getCode = async function (msg, url) {
	...
	if (!rawFiles) {
		msg.respondWithMention(`I don't know the paste service at "${url}". ljharb, ping!`);
	...
	const files = await Promise.all(Object.entries(rawFiles).map(([extension, value]) => {
		...
		const transform = typeof value.transform === 'function' ? value.transform : (x) => x;

		console.log({
			extension, transformedUrl, url,
		});
...
module.exports = async function repastePlugin(msg) {
	...
	if (words[0] === 'unpaste') {
		...
		if (msg.config.githubToken) {
			return deleteGist({ githubToken: msg.config.githubToken, id: words[1] })
				.then(() => {
	...
	if (url) {
		return getCode(msg, url).then(
			async (res) => {
				...
				if (res.html) {
					files['code.html'] = res.html;
				}
				if (res.css) {
					files['code.css'] = res.css;
				}
				...
				createGist({
					...
					.catch((err) => {
						if (err && err.gistError) {
							...
							msg.respondWithMention(`Failed due to an unknown error. ljharb, ping! ${err.message}`);
							console.error(err);
						}
					});
			},
