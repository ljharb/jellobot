...
const fixLanguage = async function (origRes, lastRedirect) {
	let res = origRes;
	...
	urlParts.pathname = urlParts.pathname.replace(
		/^\/(?:\w+)(\/docs\/)/, // eslint-disable-line prefer-named-capture-group
		(m, rest) => `/en-US${rest}`,
	);
...
const fixRedirect = async function (res) {
...
module.exports = async function mdnPlugin(msg) {
	...
	msg.respondWithMention(`${isDeprecated ? 'DEPRECATED ' : ''}${description.slice(0, 350)}${
