...
const {
	Script, SourceTextModule, createContext,
} = require('vm');
...
const exposeBuiltin = function (name) {
	const setReal = (val) => {
		delete global[name];
		global[name] = val;
	};
...
async function run(code, environment, timeout) {
	switch (environment) {
		...
		case 'node-cjs': {
			...
			builtinModules.forEach(exposeBuiltin);
		...
		case 'deno': {
			 ...
			 */
			const exec = function exec(cmd, args, {
				env, stdin, timeout: timeout2,
			} = {}) {
				let data = '';
