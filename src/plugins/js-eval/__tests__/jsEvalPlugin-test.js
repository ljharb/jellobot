...
const testEval = async function (message, opts) {
...
maybeTest('jsEvalPlugin exposes node core modules', async (t) => {
	const output = await testEval('n> fs.writeFileSync(\'foo\', \'..\'); process.nextTick(() => fs.unlinkSync(\'foo\')); child_process.execSync(\'cat foo\')+\'\'');
	t.equal(output, '(okay) \'..\'');
